...
- `miss_reason` is a closed enum: `no_candidates`, `no_402_envelope`, `no_payto`, `reachable_200`, `probe_timeout`, `quote_expired`, `invalid_need`, `upstream_5xx`, `ssrf`, `no_input_schema`, `constraints_unmet`, `probe_budget_exhausted`, `probe_limit_reached`. HTTP 402 with no usable payTo is `no_payto` (typed miss, not retry-pay). Probe budget is under 60s; a hang returns **503** JSON immediately. `stop_reason` and `probe_ceiling` say why probing stopped.
