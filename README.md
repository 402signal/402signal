...
The routing authorization is **$0.003 USDC** (`3000` atomic, 6 decimals) on every rail. It settles only when a valid live eligible route is found. Normal typed misses are not settled. Seller payment is separate. Bazaar is echoed on successful settlement so catalogs can index.
