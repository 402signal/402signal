...
402Signal is the independent check an AI agent makes right before spending. It finds the strongest x402 route across Base, Solana, and Algorand, verifies it live, and shows the evidence behind the choice. Your agent keeps the wallet. The decision history is committed to an append-only log with Falcon PQ anchoring on Algorand MainNet (production epoch `mainnet-v1`). MainNet Falcon broadcasting is not enabled. Awaiting first confirmed MainNet checkpoint. Falcon authorizes a checkpoint transaction, not a merchant payment. This is not a PQ payment rail and not a claim that the product is fully quantum-proof.
...
- `GET /pq/log/checkpoint` and `GET /pq/log/tile/*` are an **experimental** C2SP transparency log (tlog-checkpoint@v1.0.0 + tlog-tiles@v0.1.0). Production origin is `402signal.com/pq/log/mainnet-v1` (epoch `mainnet-v1`, Fly DB `/data/pq-log-mainnet.sqlite`). The historical TestNet archive origin `402signal.com/pq/log` stays on `/data/pq-log.sqlite` and is not the live production log. Production log identity is Algorand MainNet. MainNet Falcon broadcasting is not enabled. Awaiting first confirmed MainNet checkpoint. `LIVE402_PQ_FALCON_BROADCAST` is a 402signal (router) env, default unset. When set to `1` the router may POST a signer-approved SignedTxn to pinned TestNet algod. 402security must GO before anyone sets it to `1`. The isolated signer never reads BROADCAST and never POSTs. Falcon SK must never live on 402signal. Automatic MainNet submit stays off. `last_confirmed` is persisted only after an independent fetch+verify of the confirmed txn. Signing or POST success is not confirmation. Paid `POST /route` does not wait for chain inclusion. Falcon authorizes a checkpoint transaction, not a merchant payment. Paid `POST /route` may include optional `pq_trust.transparency` `{status: pending|logged_uncheckpointed|unavailable, state, log_origin, index, checkpoint_size, receipt, reveal}`. `pending` means a durable leaf plus a signed checkpoint (state `checkpoint_signed`). `logged_uncheckpointed` means the leaf is durable without a signed checkpoint. `unavailable` means append failed (not pending). Never say signed if there is no checkpoint. Newly generated route receipts use `402signal.route_decision.v3`: the public leaf is type, minute-rounded ts, nonce, and sha256 commitment only. Historical v1 and v2 leaves stay verifiable with their original semantics. A public leaf is not a claim of anonymous or unlinkable traffic. The customer `reveal` holds private evidence, salt, expected commitment, and event version. `verify_route_receipt()` checks event version, reveal, commitment, leaf_hash, inclusion, and the Ed25519 checkpoint. `receipt.leaf_hash` is present so `verify_receipt()` can still round-trip. `payment_authorization.pq_native` is always false. No `/trust` page. The homepage PQ section renders only when `last_confirmed` has a real independently confirmed txid. Production MainNet-epoch signing uses Fly secret `LIVE402_PQ_LOG_SK_MAINNET` only (never baked into git; never auto-generated on boot; never a silent fallback to `LIVE402_PQ_LOG_SK`). 402security must GO before that Fly secret is set.
- `POST /route` and JSON `GET /route` (the HTTP 402 crawler path) share the same in-memory limiter (~60/min per IP). Browser `GET /route` with `Accept: text/html` is the human page and does not consume that quota. User-Agent does not grant extra quota. On Fly the limiter key is `Fly-Client-IP`; otherwise the socket peer. `X-Forwarded-For` is not trusted. Rate-limit maps and per-host probe semaphores are TTL/LRU bounded. `GET /preview` and unpaid MCP `tools/call preview` share a looser limiter (~180/min per IP, at least 2× the route cap). `GET /pulse` and `GET /rails` each have their own ~180/min per-IP limiter (same ballpark as preview, still looser than paid `/route`). `GET /health` stays unlimited `{ok:true}`. `429` when exceeded. Production request logs are request id, method, path only, status, latency, and a coarse endpoint. Query strings, need, policy, `/preview` search, `/validate` target URL, request JSON, `PAYMENT-SIGNATURE`, `X-PAYMENT`, payment payloads, and seller response bodies are not logged. Settlement logs are `settlement_success=` plus rail and request id, not full txids. Responses send `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer`, `Strict-Transport-Security: max-age=31536000` (no includeSubDomains; www is a CNAME and Fly has no extra hostnames), and `Content-Security-Policy: default-src 'none'; script-src 'self'; connect-src 'self'; style-src 'self'; img-src 'self' data:; base-uri 'self'; frame-ancestors 'none'`. script-src stays `'self'` (no CDN, no vendor wallet scripts). connect-src is `'self'` only; homepage Base pay POSTs `/route`. HEAD 200 on `/llms.txt` `/openapi.json` `/mcp.json` `/preview` `/rails` `/pulse`. Payment resource / OpenAPI `servers` / MCP resource are pinned to `https://402signal.com` (Host is not reflected). Probe DNS uses a bounded `getaddrinfo` pool (2s); the TCP/TLS connection is pinned to those SSRF-checked public IPs with TLS SNI and HTTP Host set to the original hostname (re-pinned on redirects). Seller-declared catalog bodies are never POSTed; unjustified POST `{}` is skipped; required-body misses are `unsafe_to_probe`. Paid `/route` uses one advertised-timeout deadline for verify, probe, and settle.
...
| `LIVE402_PQ_LOG_DB` | `/data/pq-log-mainnet.sqlite` on Fly (`/tmp` fallback) | Experimental C2SP log for production epoch `mainnet-v1`. Separate file from catalog, history, and the TestNet archive `/data/pq-log.sqlite`. **Not HTTP-exposed** as a sqlite dump; read API is `/pq/log/*` only. |
| `LIVE402_PQ_LOG_VKEY` | unset | Ed25519 log verifier key (public). MainNet epoch sets this from the public half of `LIVE402_PQ_LOG_SK_MAINNET`. TestNet epoch uses `LIVE402_PQ_LOG_SK`. Never a private key. |
| `LIVE402_PQ_LOG_SK` | unset | TestNet-epoch Ed25519 log signing seed only: raw 32-byte seed as hex, or PKCS8 PEM. Fly secret only. Never commit, never paste into chat, never send this value to an assistant. Production MainNet epoch does not load this secret. If unset (or `LIVE402_PQ_LOG=0`), append may still produce `logged_uncheckpointed`; signed receipts stay unavailable. Malformed value fails closed (no random key; `/route` still serves unless `require_transparency`). Boot never generates a key. |
| `LIVE402_PQ_LOG_SK_MAINNET` | unset | Production MainNet-epoch Ed25519 seed only. Code rejects silent fallback to `LIVE402_PQ_LOG_SK`. Fly secret only. Never commit, never paste into chat. An admin sets `fly secrets set LIVE402_PQ_LOG_SK_MAINNET=…`; 402dev never holds it. 402security must GO before this secret is set. |
| `LIVE402_PQ_FALCON_ADDRESS` | unset | Public TestNet Falcon address for the historical archive. Never a private key. Not the Fly production default. |
| `LIVE402_PQ_FALCON_NETWORK` | `mainnet` in fly.toml | Production live identity is MainNet. The TestNet broadcast flag never sends MainNet. |
...
| `LIVE402_PQ_FALCON_MAINNET_ADDRESS` | fly.toml (public MainNet address) | Public MainNet Falcon f1 address. Never a private key. |
...
`fly.toml` sets `app = "402signal"`, `internal_port = 8080`, `auto_stop_machines = "off"`, `min_machines_running = 1` on the **app** HTTP process (shared-cpu-1x 1GB). One app process. Do not deploy, `fly scale`, or set secrets from this PR. `LIVE402_PQ_FALCON_BROADCAST` stays unset.
