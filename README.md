...
| `LIVE402_PQ_LOG_DB` | `/data/pq-log-mainnet.sqlite` on Fly (`/tmp` fallback) | Production C2SP log (epoch `mainnet-v1`). Separate from catalog, history, and the TestNet archive `/data/pq-log.sqlite`. **Not HTTP-exposed** as a sqlite dump; read API is `/pq/log/*` only. Ross-only empty reset: `docs/runbooks/mainnet-prelaunch-reset.md`. |
| `LIVE402_PQ_LOG_VKEY` | unset | TestNet Ed25519 log verifier key (public). Never a private key. |
| `LIVE402_PQ_LOG_VKEY_MAINNET` | unset | MainNet Ed25519 log verifier key (public). Set on boot from the public half of `LIVE402_PQ_LOG_SK_MAINNET`. Never a private key. |
| `LIVE402_PQ_LOG_SK` | unset | Optional TestNet Ed25519 log signing seed: raw 32-byte seed as hex, or PKCS8 PEM. Fly secret only. Never commit, never paste into chat, never send this value to an assistant. If unset (or `LIVE402_PQ_LOG=0`), append may still produce `logged_uncheckpointed`; signed receipts stay unavailable. Malformed value fails closed (no random key; `/route` still serves unless `require_transparency`). Boot never generates a key. Retire from the live router after the MainNet reset. |
| `LIVE402_PQ_LOG_SK_MAINNET` | unset | Fresh MainNet-epoch Ed25519 seed only. Code rejects silent fallback to `LIVE402_PQ_LOG_SK`. Install via stdin/file (`NAME=-`); never a CLI secret argument. Never set from this PR. |
| `LIVE402_PQ_FALCON_ADDRESS` | fly.toml (public TestNet address) | Public Algorand address for Falcon anchors. Never a private key. |
| `LIVE402_PQ_FALCON_NETWORK` | `mainnet` in fly.toml | Live identity is MainNet. TestNet `LIVE402_PQ_FALCON_BROADCAST` never sends MainNet. |
...
| `LIVE402_PQ_FALCON_MAINNET_ADDRESS` | fly.toml (public MainNet address) | Public MainNet Falcon f1 address. Distinct from the archived TestNet address. Never a private key. |
