...
- `GET /health` is **HTTP 200** `{ "ok": true }` for Fly checks. Not a paid listing.
- `GET /pulse` is a **free** JSON snapshot of sample lookups (Base / Solana / Algorand). Allowlisted catalog hosts only. Query params are ignored — no caller-supplied URLs. Cached ~15s. Prices are dollars (USDC 6 decimals: `10000` → `$0.01`). Includes `samples` (up to 4 per chain).
...
- Discovery (all free): `GET /openapi.json`, `GET /mcp.json`, `GET /.well-known/x402`, `GET /.well-known/x402.json`, `GET /robots.txt`, `GET /llms.txt`. Paid `POST /route` is documented with `x-payment-info` and HTTP 402.
