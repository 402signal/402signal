...
```json
{ "need": "erc20 token balance", "url": "https://example.com/x402/balance" }
```
...
- `miss_reason` is a closed enum: `no_candidates`, `no_402_envelope`, `reachable_200`, `probe_timeout`, `quote_expired`, `invalid_need`, `upstream_5xx`, `ssrf`, `no_input_schema`. Probe budget is under 60s; a hang returns **503** JSON immediately.
- `GET /health` is **HTTP 200** `{ "ok": true }` for Fly checks. Not a paid listing. Not a rails dump.
- `GET /preview?need=` is an unpaid cache preflight (`not_probed: true`, hits + prices + freshness). It does not probe and does not charge. Paid `POST /route` remains the fail-closed 402 probe.
...
- GET `/` homepage is plain English. Raw JSON is behind “Show technical details”. Footer is 402signal.com / @402Signal. Example needs are clickable chips. Injected wallet (window.ethereum): Pay $0.01 on Base signs one EIP-3009 authorization and POSTs PAYMENT-SIGNATURE. Pay $0.01 on Algorand (Pera / Lute) signs a USDC axfer in an atomic group and POSTs PAYMENT-SIGNATURE; the button is not hidden behind window.ethereum. Sample chips only prefill, never auto-pay. Solana stays agent/CLI. A short “for agents” box shows `POST https://402signal.com/route` plus links to `/openapi.json` and `/.well-known/x402.json`. Sample lookups fetch `/pulse` `samples` (not a theme catalog).
...
- `POST /route` is rate-limited in memory (~60/min per IP, higher burst for Coinbase / PayAI / GoPlausible user-agents). `GET /preview` and unpaid MCP `tools/call preview` share a looser limiter (~180/min per IP, at least 2× the route cap). `GET /pulse` and `GET /rails` each have their own ~180/min per-IP limiter (same ballpark as preview, still looser than paid `/route`). `GET /health` stays unlimited `{ok:true}`. `429` when exceeded. Payment headers are redacted in logs. Responses send `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer`, `Strict-Transport-Security: max-age=31536000` (no includeSubDomains; www is a CNAME and Fly has no extra hostnames), and `Content-Security-Policy: default-src 'none'; script-src 'self'; connect-src 'self' https://wallet-connect-[a-h].perawallet.app wss://wallet-connect-[a-h].perawallet.app; style-src 'self'; img-src 'self' data:; base-uri 'self'; frame-ancestors 'none'`. script-src stays `'self'` (vendor `/algosdk.min.js`, `/pera.js`, `/lute.js`; no CDN). connect-src prefers `'self'`; the only pinned extras are Pera WalletConnect v1 bridges `wallet-connect-[a-h].perawallet.app` (https + wss) so iPhone Pera can sign. The browser does not call algod or the facilitator — `suggestedParams` ride on the unpaid 402 extra, and the paid retry is POST `/route`. Lute uses a popup to lute.app (postMessage, not connect-src). Payment resource / OpenAPI `servers` / MCP resource are pinned to `https://402signal.com` (Host is not reflected). Probe DNS (`getaddrinfo`) times out in 2s.
...
live402/static/     GET / homepage + vendor JS (algosdk.min.js, pera.js, lute.js)
