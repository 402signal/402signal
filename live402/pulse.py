...
def dashboard_html(payload: dict | None = None) -> str:
    ...
    boot = json.dumps(data, separators=(",", ":")).replace("<", "\\u003c")
...
<body>
  ...
  <script id="pulse-boot" type="application/json">{boot}</script>
  <script>
  (function () {{
    ...
    try {{
      var boot = document.getElementById("pulse-boot");
      if (boot && boot.textContent) render(JSON.parse(boot.textContent));
    }} catch (e) {{}}
    poll();
